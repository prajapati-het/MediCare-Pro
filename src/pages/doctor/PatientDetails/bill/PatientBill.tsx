...
import { Receipt, Download, Printer, ArrowLeft, FileImage, FileType, FileText, ChevronDown } from "lucide-react";
import { Button } from "@/components/ui/button";
import { useState } from "react";
import html2canvas from "html2canvas";
import jsPDF from "jspdf";
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger, DropdownMenuSeparator, DropdownMenuLabel } from "@/components/ui/dropdown-menu";
import { useCreateBillMutation, useGetBillsByPatientQuery, useGetDoctorDetailsQuery, useGetDoctorPatientsQuery, useGetPatientByIdQuery } from "@/redux/slices/api";
...
export default function PatientBill() {
  ...
  useGetDoctorPatientsQuery(String(currentUser.id ?? null), { skip: currentUser.id == null });
  currentUser = doctor;

  const { data: billData, refetch: refetchBill } = useGetBillsByPatientQuery(
    Number(patient?.id), { skip: !patient?.id }
  );
  ...
  const billProps = {
    patient, currentUser, medicines, consultationFee, labTests,
    subtotal, tax, total, billDate, currentBillNumber, reportUrl,
    status, paymentMethod, notes, taxPercent,
  };
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-44">
                  <DropdownMenuLabel className="text-xs text-slate-400 font-normal pb-1">Export as</DropdownMenuLabel>
                  ...
                  <DropdownMenuItem onClick={() => handleDownload("png")} className="gap-2.5 cursor-pointer">
                    <FileImage className="h-4 w-4 text-blue-500" /> PNG Image
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => handleDownload("jpeg")} className="gap-2.5 cursor-pointer">
                    <FileType className="h-4 w-4 text-amber-500" /> JPEG Image
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <div key={label} className={`rounded-xl border px-4 py-3.5 ${accent ? "bg-blue-600 border-blue-500 text-white" : "bg-white border-slate-200"}`}>
                <p className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${accent ? "text-blue-200" : "text-slate-400"}`}>{label}</p>
                <p className={`text-sm font-bold truncate ${accent ? "text-white" : "text-slate-800"}`}>{value}</p>
                <p className={`text-xs mt-0.5 truncate ${accent ? "text-blue-200" : "text-slate-400"}`}>{sub}</p>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 overflow-hidden shadow-sm bg-white">
            <div className="flex items-center gap-2.5 px-6 py-3 bg-slate-50 border-b border-slate-200">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Bill Preview</span>
